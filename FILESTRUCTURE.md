...
* Python source files:
  * The Python package source files are located in the `src/pooch_figshare` directory.
  * `tests/test_pooch_figshare.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `pooch_figshare`.
