...
def test_initialize(data_repo_tester):
    ...
    ).assert_repo_does_initialize(archive_path="/somevalue/abc")

    # TESTCASE 2: With invalid archive_path and invalid base url -> repo doesnt get initialized
    data_repo_tester(
        archive_base_url="https://figggshare.com"
...
@pytest.mark.parametrize(
    "always_mock,version,search_json_resp,details_json_resp,filename,result",
    download_url_testcases,
)
def test_download_url(
    ...
    with repo_tester.endpoint_mocker(always_mock=always_mock) as m:
        ...
        m.get(
            f"https://api.figshare.com/v2/articles?doi=10.6084/m9.figshare.14763051{version}",
            json=search_json_resp,
        )
        m.get(
            "https://api.figshare.com/v2/articles/14763051/versions/1",
            json=details_json_resp,
        )
        ...
        repo_tester.initialize_repo(
            doi=f"10.6084/m9.figshare.14763051{version}",
...
@pytest.mark.parametrize(
    "always_mock,search_json_resp,details_json_resp,result", create_registry_testcases
)
...
@pytest.mark.parametrize(
    "always_mock,search_json_resp,details_json_resp,result", licenses_testcases
)
def test_licenses(
    data_repo_tester, always_mock, search_json_resp, details_json_resp, result
):
